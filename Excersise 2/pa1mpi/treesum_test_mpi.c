 ...
 */
char *treesum_test1() {
  int local_val = my_rank + 1;

    int result = global_sum(local_val, my_rank, no_proc, comm);

    if (my_rank == 0) {
        int expected = no_proc * (no_proc + 1) / 2;
        mu_assert("tree sum incorrect", result == expected);
    }
